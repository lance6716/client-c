...
void RegionClient::onRegionError(Backoffer & bo, RPCContextPtr rpc_ctx, const errorpb::Error & err) const
{
    if (err.has_not_leader())
    {
        ...
        if (not_leader.has_leader())
        {
            ...
            log->information("not leader but has leader, region_ver_id=" + rpc_ctx->region.toString() + ", new leader peer_id=" + std::to_string(not_leader.leader().id())
        ...
        else
        {
            ...
            // the region from PD.
            log->information("not leader but doesn't have new leader, region_ver_id=" + rpc_ctx->region.toString());
